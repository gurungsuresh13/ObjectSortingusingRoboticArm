...
BIN_POSITIONS = {
    ...
    "Green Cube": {"base": 120, "elbow": 40, "wrist": 25, "pivot": 70},
    "Green Cylinder": {"base": 120, "elbow": 40, "wrist": 25, "pivot": 70},
    "Green Pyramid": {"base": 120, "elbow": 40, "wrist": 25, "pivot": 70},
...
def move_robot_arm(position_index):
    ...
    smooth_move_servo(1, position['elbow']+30)
...
def on_message(client, userdata, message):
    try:
        ...
        if label and x is not None and y is not None:
            ...
            if matching_coord and label in BIN_POSITIONS:
                print(f"Matching coordinate found at index {matching_coord['position_index']}.")
                # Move to the coordinate's position, pick up the object
                move_robot_arm(matching_coord['position_index'])
                # Then move to the bin based on the label and release the object
                move_to_bin_and_release(label)
            else:
                print(f"No matching coordinate or bin position found for label '{label}'.")
        else:
            print("Received data does not contain a valid label or coordinates.")
    except Exception as e:
        print(f"Error processing message: {e}")

# Function to check if received coordinates are within tolerance of predefined ones
def is_within_tolerance(det_coord, pre_coord):
    return all(abs(det_coord[k] - pre_coord[k]) <= TOLERANCE for k in det_coord)
...
# Initialize the robot arm (if this is necessary before receiving any messages)
