...
def draw_centroids_on_image_and_get_labels(output_image, json_results, scale_factor=25):
    ...
    for obj in data:
        ...
        cv2.putText(output_image, f"{label}: {cx_scaled}, {cy_scaled}", (cx - 40, cy + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # Capture one frame when 'c' is pressed
            if key == ord('c'):
                ...
                output_image, coordinates_labels = draw_centroids_on_image_and_get_labels(rendered_images[0], json_results, scale_factor=25)
                ...
                cv2.imshow("Captured Frame", cv2.cvtColor(output_image, cv2.COLOR_RGB2BGR))
